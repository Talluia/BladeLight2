...
bool VRMConverter::ConvertModel(UVrmAssetListObject *vrmAssetList, const aiScene *mScenePtr) {
	...
	static int boneOffset = 0;
	{
		...
		//k->MergeAllBonesToBoneTree(src);
		{
			...
			UVrmSkeleton *k_tmp = VRM4U_NewObject<UVrmSkeleton>(GetTransientPackage(), NAME_None, EObjectFlags::RF_Public | RF_Transient);
	...
	//USkeleton* NewAsset = VRM4U_NewObject<USkeleton>();
